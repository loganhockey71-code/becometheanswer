...
**BecomeTheAnswerBand** — Full-width band, Cloud or Ink background, book cover image slot (client to supply; placeholder state until then) + description + 3 micro-points + primary CTA (download first chapter) + secondary CTA (pre-order). Built once, instanced per page with page-specific CTA targets as props — matches the blueprint's explicit "build as a Component, edit in one place" instruction.

**RegisterInterestForm (CTA 1)** — Fields per the General doc: first name, last name, work email, brand (optional), key competitor (optional), company (optional), form-of-interest dropdown (AI visibility report / Masterclasses / Keynote speaker / Consultancy / Other + free text). Client-side validation (required fields, email format). Submit handler is a stubbed function that logs the payload — real ESP wiring (MailerLite/ConvertKit) happens in Phase 4. Rendered inline in the Hero section on Home for Phase 1 (not yet a shared modal — that reuse pattern gets generalized when other CTA1 entry points are built in Phase 2+).
